...
class Watermark_Hotlink_Protection_Plugin{
	...
	private $plugin_title = "Watermark Hotlink Protection";
	
	//page name, also will be used as option name to save all options
	private $plugin_name = "watermark-hotlink-protection";
	
	//will be used as option name to save all options
	private $setting_name = "watermark-hotlink-protection-settings";	
	...
	public function __construct() {
		...
		$this->tools = new Watermark_Hotlink_Protection_Tools;
		$this->tools->opt = $this->opt;
		
		
		if(isset($_GET['action']) && isset($_GET['page']) && "watermark_preview" == $_GET['action'] && "watermark-hotlink-protection-settings" == $_GET['page'] ){
			$this->tools->do_watermark_preview();
			die();
		}
		

		//check plugin settings and check cache
		add_action('admin_init', array($this, 'clear_image_cache_settings_updated'), 1);
		
		//add_action('init', array($this->tools, 'watermark_cache_manager'), 1);
		
		
		add_action('init', array($this->tools, 'do_hotlink_watermark'));
		...
		add_action('plugin_action_links_' . plugin_basename(WHP_LOADER), array(&$this, 'add_plugin_actions'));
	...
	public function clear_image_cache_settings_updated(){
		
		$watermark_cache = $this->opt['watermark_settings']['image_cache'];
		
		$cache_dir_path = ABSPATH . "wp-content/hotlink-image-cache/";
		
		if(isset($_GET['page']) && isset($_GET['settings-updated']) && $_GET['page'] == 'watermark-hotlink-protection-settings' && $_GET['settings-updated'] == 'true' ){
			
			//clear cache when saving plugin settings
	...
	public function watermark_add_rewrite_rules(){
		...
		if(isset($_GET['page']) && isset($_GET['settings-updated']) && $_GET['page'] == 'watermark-hotlink-protection-settings' && $_GET['settings-updated'] == 'true' ){
			...
			if(in_array($this->opt['watermark_settings']['image_cache'], array("basic", 'disabled'))){
				...
				$rules[] = "RewriteCond %{HTTP_REFERER} !^http(s)?://(www\.)?".$site_url." [NC]";
				$rules[] = "RewriteRule (.*)wp-content/uploads/(.*).(jpe?g|gif|png) $1wp-admin/options-general.php?page=watermark-hotlink-protection-settings&action=add_hotlink_watermark&src=$2.$3 [L,NC]";
			
				if(file_exists(ABSPATH . '.htaccess'))		
					insert_with_markers(ABSPATH.'.htaccess', 'WatermarkHotlinkProtection', $rules);
	...
	public function settings_page_init() {
		 ...
		 $this->settings_page  = new Watermark_Hotlink_Protection_Settings_Page( $this->setting_name );
	...
	public function check_plugin_settings(){
		...
		if( isset($_GET['page'])  && $_GET['page'] == "watermark-hotlink-protection"  ){
			
			if(false === get_option($this->setting_name)){
				
				$link = admin_url()."options-general.php?page=watermark-hotlink-protection-settings&tab=watermark_settings";
    ...
    function get_settings_fields() {
		...
		$image_watermark_fields = array(
			...
			array(
				...
				'label' => __( 'Watermark Image Vertical Position', $this->plugin_name ),
			 	'desc' => __( "Enable Image Watermark Vertical Position Adjustnment.<br>(Feature Available in Ultra Version Only, <a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/' target='_blank'>Click Here for More Information!</a>)", $this->plugin_name ),
                'action' => 'Enable',
				'type' => 'checkbox',
                'enabled' => 'false'
			),
			array(
				...
				'label' => __( 'Watermark Image Horizontal Position', $this->plugin_name ),
			 	'desc' => __( "Enable Image Watermark Horizontal Position Adjustnment.<br>(Feature Available in Ultra Version Only, <a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/' target='_blank'>Click Here for More Information!</a>)", $this->plugin_name ),
                'action' => 'Enable',
				'type' => 'checkbox',
                'enabled' => 'false'
			),
			array(
				...
				'desc' => __( "Enable Watermark Resampling which will result in Higher Quality watermarks.<br>(Feature Available in Ultra Version Only, <a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/' target='_blank'>Click Here for More Information!</a>)", $this->plugin_name ),
				'action' => 'Enable',
				'type' => 'checkbox',
				'enabled' => 'false'
			)
		...
		$text_watermark_fields = array(
			...
			array(
				...
				'desc' => 'Configure the Watermark Text Vertical Position (Percentage)',
			 	'desc' => __( "Enable Text Watermark Vertical Position Adjustnment.<br>(Feature Available in Ultra Version Only, <a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/' target='_blank'>Click Here for More Information!</a>)", $this->plugin_name ),
                'action' => 'Enable',
				'type' => 'checkbox',
                'enabled' => 'false'
			),
			array(
				...
				'label' => __( 'Watermark Text Horizontal Position', $this->plugin_name ),
			 	'desc' => __( "Enable Text Watermark Horizontal Position Adjustnment.<br>(Feature Available in Ultra Version Only, <a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/' target='_blank'>Click Here for More Information!</a>)", $this->plugin_name ),
                'action' => 'Enable',
				'type' => 'checkbox',
                'enabled' => 'false'
			)
		...
		$settings_fields = array(
			...
			array(
					...
					'desc' => __( "Select a Watermark Type.<br>(Upgrade to <a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/' target='_blank'>Watermark Hotlink Protection Ultra</a> for <b>Text and Image Watermarks!</b>)", $this->plugin_name ),
                    'type' => 'radio',
                    'options' => array(
				...
				array(
					...
					'desc' => __( "Adjustable JPEG image output quality can adjust the size and quality of the finished images.<br>(Feature Available in Ultra Version Only, <a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/' target='_blank'>Click Here for More Information!</a>)", $this->plugin_name ),
					'type' => 'checkbox',
					'action' => 'Enable',
					'enabled' => 'false'
				)
	...
	private function get_settings_sidebar(){
		...
		$plugin_resources = "<p><a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/' target='_blank'>Plugin Homepage</a></p>
			<p><a href='http://mywebsiteadvisor.com/support/'  target='_blank'>Plugin Support</a></p>
			<p><b><a href='http://wordpress.org/support/view/plugin-reviews/watermark-hotlink-protection-plugin?rate=5#postform'  target='_blank'>Rate and Review This Plugin</a></b></p>";
		...
		$upgrade = "	<p>
			<b><a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/'  target='_blank'>Upgrade to Watermark Hotlink Protection Ultra!</a></b><br />
			<br />
			<b>Features:</b><br />
			...
			<p>-<a href='http://mywebsiteadvisor.com/tools/wordpress-plugins/simple-watermark/' target='_blank'>Simple Watermark</a></p>
			</p>";
	...
	public function admin_help(){
		...
		//if ($screen_id == $this->page_menu) {
			...
			$faqs .= "<b><a href='http://MyWebsiteAdvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/' target='_blank'>Upgrade to Watermark Hotlink Protection Ultra</a></b>";
			$faqs .= "</p>";
	...
	public function add_plugin_links($links, $file) {
		if($file == plugin_basename(WHP_LOADER)) {
			$upgrade_url = 'http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/';
	...
	public function show_plugin_upgrades(){
		...
		$html .= "<script>
			...
			function  trans_watermark_upgrade(){
        		window.open('http://mywebsiteadvisor.com/tools/wordpress-plugins/watermark-hotlink-protection/');
        		return false;
			}
			...
			function  trans_watermark_learn_more(){
        		window.open('http://mywebsiteadvisor.com/tools/wordpress-plugin/watermark-hotlink-protection/');
        		return false;
			}
		...
		$html .= "</form><h2>Upgrade to Watermark Hotlink Protection Ultra Today!</h2>";
		...
		$html .= "<p class='submit'>";
		$html .= "<input type='submit' class='button-primary' value='Upgrade to Watermark Hotlink Protection Ultra &raquo;' onclick='return trans_watermark_upgrade()'> &nbsp;";
