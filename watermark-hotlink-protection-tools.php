...
class Watermark_Hotlink_Protection_Tools{
	...
	function do_hotlink_watermark(){
	
		if(isset($_GET['action']) && $_GET['action'] == "add_hotlink_watermark"){
			...
			if(in_array($mime_type['ext'], $allowed_types)){
				...
				$cache_dir_path = ABSPATH . "wp-content/hotlink-image-cache/uploads/" . $original_path_info['dirname'];
				...
				if (!file_exists($cache_img_path)) {	
					...
					if($watermark_cache == "basic"){
						...
						//$image = $this->get_image_resource($cache_img_path, $mime_type['type']);	
						$image_location = $cache_img_path;
				}
